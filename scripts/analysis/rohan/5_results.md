...
#Heterozygosity
echo -e "chrom\tend\tvalid_sites\th\thlow\thhigh\tsample\tpop" > het.txt
for i in $(cat $scripts/hKIWA_IDS_e759877.txt); do
  zcat ${i}.hEst.gz \
  | awk -v sample="$i" 'NR>1 {print $1,$3,$4,$5,$7,$8,sample,"hKIWA"}' OFS="\t" \
  >> het.txt
done

for i in $(cat $scripts/cKIWA_IDS.txt); do
  zcat ${i}.hEst.gz \
  | awk -v sample="$i" 'NR>1 {print $1,$3,$4,$5,$7,$8,sample,"cKIWA"}' OFS="\t" \
  >> het.txt
done
