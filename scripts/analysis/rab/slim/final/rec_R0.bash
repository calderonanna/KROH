...
bcftools reheader --samples $scripts/hKIWA_slim_IDS.txt $work/hKIWA_rec.vcf -o $work/hKIWA_rec_reheadered.vcf
bcftools reheader --samples $scripts/cKIWA_slim_IDS.txt $work/cKIWA_rec.vcf -o $work/cKIWA_rec_reheadered.vcf
...
bcftools view \
  ...
  -H \
  $work/rec.vcf.gz | cut -f1,2 >> $work/nearlyneutral.txt
...
bcftools view \
  ...
  -H \
  $work/rec.vcf.gz | cut -f1,2 >> $work/weak.txt
...
#Split VCF by Sample
bcftools view -S $scripts/cKIWA_slim_IDS.txt $sim_vcf/rec.vcf -o $sim_vcf/rec_cKIWA.vcf
bcftools view -S $scripts/hKIWA_slim_IDS.txt $sim_vcf/rec.vcf -o $sim_vcf/rec_hKIWA.vcf
...
bcftools view \
  ...
  -i 'INFO/S>-0.0001 && INFO/S<0' \
  -H \
  $work/rec_eggs.vcf.gz | cut -f1,2 >> $work/nearlyneutral.txt
...
bcftools view \
  ...
  -i 'INFO/S<=-0.0001 && INFO/S>-0.001' \
  -H \
  $work/rec_eggs.vcf.gz | cut -f1,2 >> $work/weak.txt
