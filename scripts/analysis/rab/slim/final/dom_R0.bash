...
work="/storage/group/zps5164/default/abc6435/KROH/data/slim/vcf/dom/R0"
...
cp $scripts/dom.slim $scripts/dom_R0.slim
sed -i "s/dom\//dom\/R0\//g" $scripts/dom_R0.slim

#Run SLiM script
$slim -p $scripts/dom_R0.slim

# Reheader Sample Names
bcftools reheader --samples $scripts/hKIWA_slim_IDS.txt $work/hKIWA_dom.vcf -o $work/hKIWA_dom_reheadered.vcf
bcftools reheader --samples $scripts/cKIWA_slim_IDS.txt $work/cKIWA_dom.vcf -o $work/cKIWA_dom_reheadered.vcf
...
bcftools view \
  ...
  -H \
  $work/dom.vcf.gz | cut -f1,2 >> $work/nearlyneutral.txt
...
bcftools view \
  ...
  -H \
  $work/dom.vcf.gz | cut -f1,2 >> $work/weak.txt
...
sim_vcf="/storage/home/abc6435/SzpiechLab/abc6435/KROH/data/slim/vcf/dom/R0/eggs"
eggs="/storage/home/abc6435/SzpiechLab/bin/EGGS/bin/eggs"
work="/storage/home/abc6435/SzpiechLab/abc6435/KROH/data/slim/vcf/dom/R0/eggs"

#Split VCF by Sample
bcftools view -S $scripts/cKIWA_slim_IDS.txt $sim_vcf/dom.vcf -o $sim_vcf/dom_cKIWA.vcf
bcftools view -S $scripts/hKIWA_slim_IDS.txt $sim_vcf/dom.vcf -o $sim_vcf/dom_hKIWA.vcf
#bcftools view -S $scripts/cKIWA_IDS.txt $sim_vcf/dom.vcf -o $sim_vcf/dom_cKIWA.vcf
#bcftools view -S $scripts/hKIWA_IDS_e759877.txt $sim_vcf/dom.vcf -o $sim_vcf/dom_hKIWA.vcf
bgzip $sim_vcf/dom_cKIWA.vcf
bgzip $sim_vcf/dom_hKIWA.vcf

#Run Eggs
$eggs \
-b $emp_vcf/dSETO_auto_bi_qual_dp_KIWA_private_alternate_cKIWA.vcf.gz \
-d 0.712462,0.05 -k \
< $sim_vcf/dom_cKIWA.vcf.gz \
> $sim_vcf/dom_cKIWA_eggs.vcf.gz

$eggs \
-b $emp_vcf/dSETO_auto_bi_qual_dp_KIWA_private_alternate_hKIWA.vcf.gz \
-d 0.712462,0.05 -k \
< $sim_vcf/dom_hKIWA.vcf.gz \
> $sim_vcf/dom_hKIWA_eggs.vcf.gz

#Merge VCFs
gunzip $sim_vcf/dom_cKIWA_eggs.vcf.gz
bgzip $sim_vcf/dom_cKIWA_eggs.vcf
bcftools index $sim_vcf/dom_cKIWA_eggs.vcf.gz
gunzip $sim_vcf/dom_hKIWA_eggs.vcf.gz
bgzip $sim_vcf/dom_hKIWA_eggs.vcf
bcftools index $sim_vcf/dom_hKIWA_eggs.vcf.gz
bcftools merge \
$sim_vcf/dom_hKIWA_eggs.vcf.gz \
$sim_vcf/dom_cKIWA_eggs.vcf.gz \
-Oz -o $sim_vcf/dom_eggs.vcf.gz
bcftools index $sim_vcf/dom_eggs.vcf.gz
...
sed -i 's/chr//g' $work/intergenic.bed
echo -e "chromo\tposition" > $work/intergenic.txt 
bcftools view -R $work/intergenic.bed $work/dom_eggs.vcf.gz | grep -v ^# | cut -f1,2 >> $work/intergenic.txt 
rm -rf $work/intergenic.bed 
...
bcftools view \
  ...
  -i 'INFO/S==0' \
  -H \
  $work/dom_eggs.vcf.gz | cut -f1,2 >> $work/neutral.txt
...
bcftools view \
  ...
  -i 'INFO/S>-0.0001 && INFO/S<0' \
  -H \
  $work/dom_eggs.vcf.gz | cut -f1,2 >> $work/nearlyneutral.txt
...
bcftools view \
  ...
  -i 'INFO/S<=-0.0001 && INFO/S>-0.001' \
  -H \
  $work/dom_eggs.vcf.gz | cut -f1,2 >> $work/weak.txt
...
bcftools view \
  ...
  -i 'INFO/S<=-0.001 && INFO/S>-0.01' \
  -H \
  $work/dom_eggs.vcf.gz | cut -f1,2 >> $work/mild.txt
...
bcftools view \
  ...
  -i 'INFO/S>-0.1 && INFO/S<=-0.01' \
  -H \
  $work/dom_eggs.vcf.gz | cut -f1,2 >> $work/moderate.txt
...
bcftools view \
  ...
  -i 'INFO/S<-0.1' \
  -H \
  $work/dom_eggs.vcf.gz | cut -f1,2 >> $work/strong.txt

rm -rf $work/exon.bed

#Calculate RAB
source ~/RABvcfs_env/bin/activate
bgzip -d $work/dom_eggs.vcf.gz
rm -rf $work/dom_eggs.vcf.gz.csi
